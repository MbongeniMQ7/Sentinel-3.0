...
// Branded email sent when an owner promotes a manager to owner.
export function renderPromotionEmail(input: {
  fullName: string
  organizationName: string
  loginUrl: string
}): string {
  const { fullName, organizationName, loginUrl } = input
  const firstName = fullName.trim().split(/\s+/)[0] || "there"
...
<meta name="viewport" content="width=device-width, initial-scale=1.0" />
<meta name="color-scheme" content="light" />
<title>You're now an Owner on Sentinel-AI</title>
</head>
<body style="margin:0;padding:0;background:#EDECE7;">
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#EDECE7;padding:32px 16px;">
<tr><td align="center">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#ffffff;border:1px solid rgba(0,0,0,0.07);border-radius:20px;overflow:hidden;">
    ...
    <tr>
      <td style="padding:36px 40px 8px 40px;">
        <h1 style="margin:0;font-family:'IBM Plex Sans',Helvetica,Arial,sans-serif;font-weight:300;font-size:26px;line-height:1.2;color:#111;letter-spacing:-0.01em;">You're now an Owner, ${firstName}</h1>
        <p style="margin:14px 0 0 0;font-family:Helvetica,Arial,sans-serif;font-size:14px;line-height:1.6;color:rgba(0,0,0,0.5);">Your role at <strong style="color:#111;">${organizationName}</strong> has been upgraded to <strong style="color:#111;">Owner</strong>. You now have full access to the Sentinel-AI Workforce platform.</p>
      </td>
    </tr>
    <tr>
      <td style="padding:24px 40px 8px 40px;">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#F5F4F0;border:1px solid rgba(0,0,0,0.08);border-radius:12px;">
          <tr>
            <td style="padding:16px 20px;border-bottom:1px solid rgba(0,0,0,0.06);font-family:Helvetica,Arial,sans-serif;font-size:11px;letter-spacing:0.08em;text-transform:uppercase;color:rgba(0,0,0,0.4);">New role</td>
            <td style="padding:16px 20px;border-bottom:1px solid rgba(0,0,0,0.06);font-family:Helvetica,Arial,sans-serif;font-size:14px;font-weight:600;color:#111;text-align:right;">Owner</td>
          </tr>
          <tr>
            <td style="padding:16px 20px;font-family:Helvetica,Arial,sans-serif;font-size:11px;letter-spacing:0.08em;text-transform:uppercase;color:rgba(0,0,0,0.4);">Organization</td>
            <td style="padding:16px 20px;font-family:Helvetica,Arial,sans-serif;font-size:14px;font-weight:600;color:#111;text-align:right;">${organizationName}</td>
          </tr>
        </table>
      </td>
    </tr>
    <tr>
      <td style="padding:20px 40px 8px 40px;">
        <p style="margin:0;font-family:Helvetica,Arial,sans-serif;font-size:14px;line-height:1.6;color:rgba(0,0,0,0.6);">As an Owner you can manage sites, employees, analytics, reports and organization settings. Sign back in to see your new dashboard.</p>
      </td>
    </tr>
    <tr>
      <td style="padding:24px 40px 8px 40px;">
    ...
    <tr>
      <td style="padding:32px 40px 36px 40px;">
        ...
        <p style="margin:16px 0 0 0;font-family:Helvetica,Arial,sans-serif;font-size:11px;line-height:1.6;color:rgba(0,0,0,0.35);">
          Weren't expecting this? Contact your organization's administrator.<br/>
          Need help? Contact <a href="mailto:${SUPPORT_EMAIL}" style="color:rgba(0,0,0,0.5);">${SUPPORT_EMAIL}</a>.<br/>
          &copy; 2026 Sentinel-AI Workforce
        </p>
      </td>
